...
class Paginator:
    ...
    def __init__(
        ...
        self.pages_count = max((len(self._objects) + self.page_size - 1) // self.page_size, 1)
    ...
    def _get_kb_paginator_elements(self):
        ...
        if self.current_page == 0:
            keyboard.append(InlineKeyboardButton(text=self._symbol_fill, callback_data='paginator_show_alert'))
            keyboard.append(InlineKeyboardButton(text=self._symbol_fill, callback_data='paginator_show_alert'))
        else:
            keyboard.append(InlineKeyboardButton(
                text=self._symbol_start, callback_data="paginator_open_page_0"))
            keyboard.append(InlineKeyboardButton(
        ...
        keyboard.append(InlineKeyboardButton(text=self._formatted_text_for_button_of_current_page.format(
            page_number=(self.current_page + 1),
            pages_count=self.pages_count
        ), callback_data='paginator_update_page'))

        if self.current_page + 1 < self.pages_count:
            keyboard.append(InlineKeyboardButton(text=self._symbol_right,
                                                 callback_data=f'paginator_open_page_{self.current_page + 1}'))
            keyboard.append(InlineKeyboardButton(
                text=self._symbol_end, callback_data=f"paginator_open_page_{self.pages_count - 1}"))
        else:
            keyboard.append(InlineKeyboardButton(text=self._symbol_fill, callback_data='paginator_show_alert'))
            keyboard.append(InlineKeyboardButton(text=self._symbol_fill, callback_data='paginator_show_alert'))
        return keyboard
            ...
            for i, obj in enumerate(self._get_objects_on_current_page()):
                ...
                current_row.append(
                    InlineKeyboardButton(
                        ...
                        callback_data=self._get_callback_data_from_object_func(obj,
                                                                               i + self.current_page * self.page_size)
                    )
                )
            if current_row:
                kb.append(current_row)
